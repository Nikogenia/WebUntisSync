...
export default function WebUntis({ user, config, fetchData, router }) {
  ...
  const handleUpdateConfig = async (event) => {
    ...
    try {
      ...
      console.error('Failed to update WebUntis configuration:', response.status);
    } catch (err) {
      toast.error('Failed to update WebUntis configuration!');
    ...
    return (
      <Dialog>
        ...
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle>{config.webuntis.password_configured ? "Change" : "Configure"} Password</DialogTitle>
            <DialogDescription>
              Please provide your WebUntis login password
            </DialogDescription>
          </DialogHeader>
          <form onSubmit={(event) => handleConfigurePassword(event, password)} className="space-y-3">
          ...
          <DialogFooter>
            <DialogDescription className="text-sm text-red-500">
              Your credentials will be encrypted and stored securely. They will never leave our servers!
            </DialogDescription>
          </DialogFooter>
        </DialogContent>
      </Dialog>
          ...
          <div className="flex flex-col xl:flex-row xl:items-center space-x-3 space-y-2 xl:space-y-0">
            <div className="flex gap-3">
              {config.webuntis.password_configured && (
                <Button type="button" variant="outline" className="text-red-500 cursor-pointer" onClick={handleDeletePassword}>
                  Delete
                </Button>
              )}
              <PasswordDialog />
            ...
            {config.webuntis.password_configured ? (
              <div className="text-sm text-muted-foreground flex items-center">
                ...
                <span>{formatDate(config.webuntis.password_configured)}</span>
                <Clock className="ml-2 mr-1 h-4 w-4" />
                <span>{formatTime(config.webuntis.password_configured)}</span>
              </div>
            ) : (
