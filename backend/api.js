...
app.get("/api/auth", async (req, res) => {
  try {
    ...
    if (!user.password)
      return res.status(401).json({ name: username, fullname: user.fullname });
...
app.get("/api/logs", authenticate, refresh, async (req, res) => {
  try {
    if (!req.user) return res.status(400).json({ token: req.token });
    const before =
    ...
    if (
      ...
      return res.status(400).json({ token: req.token });
