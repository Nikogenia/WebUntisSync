...
async function uploadEvent(
  ...
  try {
    ...
    if (
      !res.data.extendedProperties ||
      !res.data.extendedProperties.private ||
      !res.data.extendedProperties.private.untisversion
    ) {
      console.warn(
        `[${name}]`,
        "Event",
    ...
    const equal = (x, y) =>
      ...
      (x === "" && y === undefined);
    const same =
      equal(res.data.summary, event.summary) &&
      ...
      equal(
        new Date(res.data.start.dateTime).getTime(),
        new Date(event.start.dateTime).getTime()
      ) &&
      equal(
        new Date(res.data.end.dateTime).getTime(),
        new Date(event.end.dateTime).getTime()
      ) &&
      equal(
        new Date(res.data.start.date).getTime(),
        new Date(event.start.date).getTime()
      ) &&
      equal(
        new Date(res.data.end.date).getTime(),
...
export async function uploadHolidays(name, api, calendarId, holidays, queue) {
  ...
  for (const holiday of holidays) {
    ...
    queue(async () =>
      uploadEvent(name, api, calendarId, `untisholi${holiday.id}`, event, 1)
    );
  }
  return tasks;
}
...
export async function uploadLessons(name, api, calendarId, lessons, queue) {
  ...
  for (const lesson of lessons) {
    try {
      ...
      queue(async () =>
        uploadEvent(name, api, calendarId, `untisless${lesson.id}`, event, 1)
      );
    } catch (err) {
