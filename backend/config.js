...
export async function loadConfig() {
    ...
    if (!newConfig) {
        ...
        if (JSON.stringify(users) !== JSON.stringify(newConfig.users)) {
...
export async function loadUserFile(user) {
    ...
    try {
        const rawData = await fs.readFile(path.join('users', 'config-' + user + '.json'), 'utf8');
        const data = JSON.parse(rawData);
        if (data.version !== '1.0') {
            console.error('Unsupported user config version', data.version, 'for', user, 'expected 1.0');
            return null;
        }
...
export async function saveUserFile(user, data) {
    ...
    try {
        ...
        await fs.writeFile(path.join('users', 'config-' + user + '.json'), rawData, 'utf8');
